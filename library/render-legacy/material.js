...
export class Material {
	...
	CreateInstance(name) {
		if (name == undefined) name = this.GetName();
		return new MaterialInstance(
			binding.Material$CreateInstance(this.internal, name),
		);
	}
	GetName() {
	...
	GetInterpolation() {
		return enumToString(
			binding.Material$GetInterpolation(this.internal),
			Interpolation,
		);
	}
	GetBlendingMode() {
		return enumToString(
			binding.Material$GetBlendingMode(this.internal),
			BlendingMode,
		);
	}
	GetVertexDomain() {
		return enumToString(
			binding.Material$GetVertexDomain(this.internal),
			VertexDomain,
		);
	}
	...
	GetMaterialDomain() {
		return enumToString(
			binding.Material$GetMaterialDomain(this.internal),
			MaterialDomain,
		);
	}
	GetCullingMode() {
		return enumToString(
			binding.Material$GetCullingMode(this.internal),
			CullingMode,
		);
	}
	GetTransparencyMode() {
		return enumToString(
			binding.Material$GetTransparencyMode(this.internal),
			TransparencyMode,
		);
	}
	/**
	...
	GetRefractionMode() {
		return enumToString(
			binding.Material$GetRefractionMode(this.internal),
			RefractionMode,
		);
	}
	GetRefractionType() {
		return enumToString(
			binding.Material$GetRefractionType(this.internal),
			RefractionType,
		);
	}
	GetReflectionMode() {
		return enumToString(
			binding.Material$GetReflectionMode(this.internal),
			ReflectionMode,
		);
	}
	GetFeatureLevel() {
	...
	GetDefaultInstance() {
		return new MaterialInstance(
			binding.Material$GetDefaultInstance(this.internal),
		);
	}
...
export class MaterialBuilder {
	Build() {
		if (this.payload == undefined) throw "Material payload is not defined.";
...
export class ParameterInfo {
	static Build(
		...
		info.parameterType =
			isSampler || isSubpass
				? undefined
				: enumToString(parameterType, UniformType);
		info.samplerType = isSampler
			? enumToString(samplerType, SamplerType)
			: undefined;
		info.subpassType = isSubpass
			? enumToString(subpassType, SubpassType)
			: undefined;
		info.count = count;
...
export class MaterialInstance {
	...
	Duplicate(materialInstance, name) {
		return new MaterialInstance(
			binding.MaterialInstance$Duplicate(materialInstance.internal, name),
		);
	}
	GetMaterial() {
	...
	SetParameterTexture(name, texture, sampler) {
		binding.MaterialInstance$SetParameter(
			this.internal,
			2,
			name,
			texture.internal,
	...
	SetScissor(left, bottom, width, height) {
		binding.MaterialInstance$SetScissor(
			this.internal,
			Math.floor(left),
			Math.floor(bottom),
			Math.floor(width),
	...
	SetSpecularAntiAliasingVariance(variance) {
		binding.MaterialInstance$SetSpecularAntiAliasingVariance(
			this.internal,
			variance,
		);
	}
	 ...
	 */
	GetSpecularAntiAliasingVariance() {
		return binding.MaterialInstance$GetSpecularAntiAliasingVariance(
			this.internal,
		);
	}
	...
	SetSpecularAntiAliasingThreshold(threshold) {
		binding.MaterialInstance$SetSpecularAntiAliasingThreshold(
			this.internal,
			threshold,
		);
	}
	 ...
	 */
	GetSpecularAntiAliasingThreshold() {
		return binding.MaterialInstance$GetSpecularAntiAliasingThreshold(
			this.internal,
		);
	}
	...
	SetStencilCompareFunction(func, face) {
		if (face == undefined) face = StencilFace.FRONT_AND_BACK;
		binding.MaterialInstance$SetStencilCompareFunction(
			this.internal,
			func,
			face,
		);
	...
	SetStencilOpDepthStencilPass(op, face) {
		if (face == undefined) face = StencilFace.FRONT_AND_BACK;
		binding.MaterialInstance$SetStencilOpDepthStencilPass(
			this.internal,
			op,
			face,
		);
	...
	SetStencilReferenceValue(value, face) {
		if (face == undefined) face = StencilFace.FRONT_AND_BACK;
		binding.MaterialInstance$SetStencilReferenceValue(
			this.internal,
			value,
			face,
		);
	...
	SetStencilWriteMask(writeMask, face) {
		if (face == undefined) face = StencilFace.FRONT_AND_BACK;
		binding.MaterialInstance$SetStencilWriteMask(
			this.internal,
			writeMask,
			face,
		);
