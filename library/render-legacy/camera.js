...
export class CameraManager {
	...
	static ProjectionMatFromFov(
		...
		if (far == undefined) far = Number.MAX_VALUE;
		return binding.CameraManager$ProjectionMatFromFov(
			entity,
			direction,
			fovInDegrees,
			aspect,
	...
	static ProjectionMatFromFocal(entity, focalInMillimeters, aspect, near, far) {
		if (far == undefined) far = Number.MAX_VALUE;
		return binding.CameraManager$ProjectionMatFromFocal(
			entity,
			focalInMillimeters,
			aspect,
			near,
	...
	static SetProjectionFromFrustum(
		entity,
		projection,
		left,
		right,
		...
		binding.CameraManager$SetProjectionFromFrustum(
			entity,
			projection,
			left,
			right,
	...
	static SetProjectionFromFromFov(
		entity,
		fovInDegrees,
		aspect,
		near,
		...
		if (direction == undefined) direction = this.Fov.VERTICAL;
		binding.CameraManager$SetProjectionFromFromFov(
			entity,
			fovInDegrees,
			aspect,
			near,
	...
	static SetLensProjection(entity, focalInMillimeters, aspect, near, far) {
		binding.CameraManager$SetLensProjection(
			entity,
			focalInMillimeters,
			aspect,
			near,
	...
	static LookAt(entity, eye, center, up) {
		if (up == undefined) up = vec3.fromValues(0, 1, 0);
	...
	static SetExposure(entity, aperture, shutterpeed, sensitivity) {
		binding.CameraManager$SetExposure(
			entity,
			aperture,
			shutterpeed,
			sensitivity,
	...
	static InverseProjectionMat(projection) {
		return binding.CameraManager$InverseProjectionMat(
			projection,
			mat4.fromValues,
		);
	}
