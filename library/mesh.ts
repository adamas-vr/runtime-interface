...
export class Mesh {
  ...
  static setVertices(handle: number, vertices: number[]): boolean {
    return Boolean(RpcClient.Call("Mesh_SetVertices", {
      meshHandle:   handle,
  ...
  static setTriangles(handle: number, indices: number[]): boolean {
    return Boolean(RpcClient.Call("Mesh_SetTriangles", {
      meshHandle:    handle,
  ...
  static setUVs(handle: number, uvs: number[]): boolean {
    return Boolean(RpcClient.Call("Mesh_SetUVs", {
      meshHandle: handle,
  ...
  static setNormals(handle: number, normals: number[]): boolean {
    return Boolean(RpcClient.Call("Mesh_SetNormals", {
      meshHandle:    handle,
  ...
  static recalcNormals(handle: number): boolean {
    return Boolean(RpcClient.Call("Mesh_RecalculateNormals", { meshHandle: handle }));
  }

  static recalcBounds(handle: number): boolean {
    return Boolean(RpcClient.Call("Mesh_RecalculateBounds", { meshHandle: handle }));
  }
}
