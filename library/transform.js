...
export class TransformManager
{
    ...
    static Create(entity, parentEntity, localTransform) {
        if (localTransform == undefined) {
            if (parentEntity == undefined)
                binding.TransformManager$Create(entity);
            else
                binding.TransformManager$Create(entity, parentEntity);
        } else {
    ...
    static SetParent(entity, parentEntity = undefined) {
        if (parentEntity)
            binding.TransformManager$SetParent(entity, parentEntity);
    }
    /**
    ...
    static GetWorldTransform(entity) {
        return binding.TransformManager$GetWorldTransform(entity, glm.mat4.fromValues);
    }
}